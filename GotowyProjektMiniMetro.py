...
while running:
                        ...
                        if line_to_delete:
                            ...
                            for train in trains:
                                if train.color == line_to_delete.color:
                                    count += 1
                                    # ======
                                    current_station = train.line.stations[train.find_station_index_by_id(train.current_station_id)]
                                    for p in train.passengers:
                                        current_station.passengers.append(p)
                                    # =======
    ...
    if not game_over:
        ...
        for station in stations:
            timer_increase_value = random.randint(1,20 + (round(score/20)))
            station.passenger_timer += timer_increase_value
            if station.passenger_timer >= PASSENGER_SPAWN_RATE:
                station.passenger_timer = 0
                if not station.add_passenger(stations):
                    game_over = True
                    print(f"Game Over! Stacja {station.shape} ({station.pos}) przepełniona.")
