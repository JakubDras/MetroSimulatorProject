...
class A2CTrainer(torch.nn.Module):
    ...
    def get_actions_and_values(self, obs_gpu: dict, masks_cpu: dict) -> tuple:
        ...
        #Pętla po 8 środowiskach (bardzo szybka pętla Pythona)
        for i in range(self.num_envs):
            # --- Akcja Wysokiego Poziomu (dla env[i]) ---
            hl_mask = torch.as_tensor(masks_cpu["high_level"][i], dtype=torch.bool, device=self.device)
            # Pobieramy logity dla i-tego środowiska
            ...
            # --- Akcje Niskiego Poziomu (dla env[i]) ---
    ...
    def _collect_rollouts(self) -> dict:
        """
        Zbiera dane przez `self.num_steps` ze wszystkich `self.num_envs` środowisk.
        Oblicza także GAE (Advantages).
        """

        # Bufor przechowujący dane (na GPU dla łatwiejszych obliczeń)
    ...
    def training_step(self, optimizer: torch.optim.Optimizer):
        ...
        # Krok 1: Zbierz dane (teraz zwraca spłaszczone tensory)
        ...
        for _ in range(self.ppo_epochs):
            ...
            for start in range(0, num_samples, self.batch_size):
                ...
                # ... (cała logika pobierania minibatchy) ...
                ...
                # ... (cała logika normalizacji i obliczania strat) ...
                ...
                last_loss = loss.item()  # Zapisz ostatnią stratę
        ...
        return {
            "avg_reward": self.last_avg_reward,
