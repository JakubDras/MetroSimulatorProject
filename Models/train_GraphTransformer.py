...
if __name__ == "__main__":
    ...
    # --- [ZMIANA] Nazwa eksperymentu ---
    EXPERIMENT_NAME = "A2C_GraphTransformer_test1"
    # -------------------------------------------------
    ...
    # --- [ZMIANA] Inicjalizacja nowego modelu ---
    model_to_train = GraphTransformerModel(
        num_node_features=num_node_features,
        hidden_dim=128,
        num_stations=num_stations,
        heads=4  # Domyślna liczba głowic dla transformera
    )
    # --- Koniec zmiany ---
    ...
    for epoch in (pbar := tqdm(range(MAX_EPOCHS))):
        ...
        if not is_frozen and epoch >= FREEZE_EPOCH:
            # Wywołujemy nową funkcję z GraphTransformerModel
